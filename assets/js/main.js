...
(function init_adsd() {
  ...
  try {
    const keep = sessionStorage.getItem('ADSD_SELECTION_ACTIVE') === '1'
    overlay.toggle(!!keep)
  } catch (_) {
  ...
  function bind_admin_bar_icon() {
    const icon = document.getElementById('adsd-icon')?.closest('#wp-admin-bar-adsd-toggle') || document.getElementById('wp-admin-bar-adsd-toggle')
    if (!icon) return
    icon.addEventListener('click', (e) => {
      e.preventDefault()
      overlay.toggle(!state.active)
      try { sessionStorage.setItem('ADSD_SELECTION_ACTIVE', state.active ? '1' : '0') } catch (_) {}
      icon.classList.toggle('adsd-active', state.active)
      return false
    })
