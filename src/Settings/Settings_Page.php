...
class Settings_Page
{
    ...
    public function render_page(): void
    {
        ...
        <div class="wrap">
            ...
            if ($notice_code) {
                $messages = [
                    'general_ok' => __('Settings saved.', 'disable-admin-ad'),
                    'logging_ok' => __('Settings saved.', 'disable-admin-ad'),
                    'reset_ok'   => __('All rules have been reset.', 'disable-admin-ad'),
                    'delete_ok'  => __('Rule deleted.', 'disable-admin-ad'),
                    'update_ok'  => __('Rule updated.', 'disable-admin-ad'),
                    'toggle_ok'  => __('Rule status updated.', 'disable-admin-ad'),
                ];
            ...
            ?>
            <h2><?php echo esc_html__('General', 'disable-admin-ad'); ?></h2>
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"
                ...
                <label title="<?php echo esc_attr__('Show the Selector button in the admin bar for allowed roles.', 'disable-admin-ad'); ?>">
                    ...
                    <?php echo esc_html__('Show “Selector” button in the admin bar', 'disable-admin-ad'); ?>
                </label>
                <div>
                    <label for="aidad-roles-box" style="display:block; font-weight:600;"
                           title="<?php echo esc_attr__('Choose which user roles can hide blocks and manage rules.', 'disable-admin-ad'); ?>"><?php echo esc_html__('Available roles', 'disable-admin-ad'); ?></label>
                    ...
                    <p class="description"><?php echo esc_html__('Unchecked roles will not see the “Selector” button and cannot modify rules.', 'disable-admin-ad'); ?></p>
                </div>
                <label title="<?php echo esc_attr__('Enable or disable writing plugin events to the log.', 'disable-admin-ad'); ?>">
                ...
                <button type="submit" class="button button-primary"
                    ...
                    <?php echo esc_html__('Save general settings', 'disable-admin-ad'); ?>
                </button>
            </form>
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"
                ...
                <button type="submit" class="button button-secondary"
                        onclick="return confirm('<?php echo esc_js(__('Are you sure you want to reset all rules?', 'disable-admin-ad')); ?>');"
                        title="<?php echo esc_attr__('Delete all rules permanently. This cannot be undone.', 'disable-admin-ad'); ?>">
            ...
            <div class="aidad-instructions"
                ...
                <h3 style="margin-top:0;">&nbsp;<?php echo esc_html__('Usage instructions', 'disable-admin-ad'); ?></h3>
                <ol style="padding-left:18px;">
                    <li><?php echo esc_html__('Make sure your role is checked in "Available roles" and the "Selector" button is enabled in the admin bar.', 'disable-admin-ad'); ?></li>
                    <li><?php echo esc_html__('In the admin area, click the "Selector" button in the top toolbar.', 'disable-admin-ad'); ?></li>
                    <li><?php echo esc_html__('Hover to highlight the block you want to hide, click it, then confirm creating a rule.', 'disable-admin-ad'); ?></li>
                    <li><?php echo esc_html__('The rule will appear in the table below. Click "Edit" to modify XPath inline and save.', 'disable-admin-ad'); ?></li>
                    <li><?php echo esc_html__('Use "Activate/Deactivate" to toggle a rule, "Delete" to remove it, and "Reset all rules" to clear all rules.', 'disable-admin-ad'); ?></li>
                </ol>
                <p class="description" style="margin-bottom:0;">
                    &nbsp;<?php echo esc_html__('Tooltips are available when hovering over settings elements.', 'disable-admin-ad'); ?></p>
            </div>
        </div>
        <?php
    }
    ...
    private function render_rules_table(array $opts): void
    {
        ...
        <table class="widefat aidad-rules-table">
            ...
            <tr>
                ...
                <th style="width:260px"><?php echo esc_html__('Info', 'disable-admin-ad'); ?></th>
            ...
            <?php foreach ($rules as $rule) :
                ...
                $period_label = $expires_at > 0
                        ? sprintf( /* translators: %s: until datetime */ esc_html__('Until %s', 'disable-admin-ad'), date_i18n(get_option('date_format') . ' ' . get_option('time_format'), $expires_at))
                ...
                <tr class="aidad-row" data-rule-id="<?php echo esc_attr((string)$rule['id']); ?>">
                        ...
                        <form class="aidad-xpath-edit" method="post"
                            ...
                            <div style="margin-top:6px;">
                                <button type="submit" class="button button-small"
                                        data-aidad-action="save"><?php esc_html_e('Save', 'disable-admin-ad'); ?></button>
                                <a href="#" class="button button-small"
                                   data-aidad-action="cancel"><?php esc_html_e('Cancel', 'disable-admin-ad'); ?></a>
                            </div>
                        </form>
                    </td>
                    ...
                    <td>
                        <a href="#" class="button button-small"
                           data-aidad-action="edit"><?php esc_html_e('Edit', 'disable-admin-ad'); ?></a>
                        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"
                            ...
                            <input type="hidden" name="rule_id" value="<?php echo esc_attr((string)$rule['id']); ?>"/>
                            <button type="submit"
                                    class="button button-small"><?php echo !empty($rule['active']) ? esc_html__('Deactivate', 'disable-admin-ad') : esc_html__('Activate', 'disable-admin-ad'); ?></button>
                        </form>
                        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"
                              style="display:inline-block;margin-left:6px;">
                            ...
                            <button type="submit" class="button button-small button-link-delete"
                                    onclick="return confirm('<?php echo esc_js(__('Delete this rule?', 'disable-admin-ad')); ?>');"><?php echo esc_html__('Delete', 'disable-admin-ad'); ?></button>
                        </form>
                    </td>
                </tr>
            <?php endforeach; ?>
